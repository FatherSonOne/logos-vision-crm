...
export async function generateColorPalette(themeDescription: string): Promise<string> {
    if (!process.env.API_KEY) return JSON.stringify({ error: "API key not configured." });
...
// --- Dashboard Briefing ---
// FIX: Update function signature to accept currentUserId and use it instead of hardcoded 'c1'.
export async function generateDailyBriefing(
  ...
  const myUpcomingTasks = tasks
    .filter(t => {
        ...
        // The `tasks` array is already filtered by current user in the component.
        return dueDate <= oneWeekFromNow && dueDate >= today && t.status !== TaskStatus.Done;
    })
    .map(t => `- Task for project '${t.projectName}': "${t.description}" due ${new Date(t.dueDate).toLocaleDateString()}.`);

  const myHighPriorityCases = cases
    .filter(c => c.assignedToId === currentUserId && c.priority === CasePriority.High && c.status !== CaseStatus.Resolved && c.status !== CaseStatus.Closed)
    .map(c => `- High-priority case: "${c.title}".`);
    
  const myProjectsAtRisk = projects
    .filter(p => p.teamMemberIds.includes(currentUserId))
    .map(p => {
        const overdueTasks = p.tasks.filter(t => new Date(t.dueDate) < today && t.status !== TaskStatus.Done).length;
        if (overdueTasks > 0) {
            return `- Project at risk: '${p.name}' has ${overdueTasks} overdue task(s).`;
        }
        return null;
    }).filter(Boolean);

  const myTodaysActivities = activities
    .filter(a => a.createdById === currentUserId && a.activityDate === todayStr && a.status === ActivityStatus.Scheduled)
    .map(a => `- ${a.type}: "${a.title}"${a.activityTime ? ` at ${a.activityTime}` : ''}.`);

  const prompt = `
    You are an expert executive assistant AI for a CRM called Logos Vision.
    Your task is to generate a personalized and action-oriented "Daily Briefing" for ${userName}.
    The current date is ${today.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}.

    Analyze the following items and create a prioritized summary. Start with the most urgent or important items.
    Your tone should be professional, concise, and helpful. Start with "Good morning, ${userName}."
    Synthesize the information into a cohesive narrative, not just a list. Group similar items. For example, "You have 3 tasks due this week, starting with..."

    **Data for Briefing:**
    
    **My High-Priority Cases:**
    ${myHighPriorityCases.join('\n') || 'None.'}

    **My Projects with Overdue Tasks:**
    ${myProjectsAtRisk.join('\n') || 'None.'}

    **My Upcoming Tasks (due within 7 days):**
    ${myUpcomingTasks.join('\n') || 'None.'}

    **My Scheduled Activities for Today:**
    ${myTodaysActivities.join('\n') || 'None.'}

    **Instructions:**
    1.  Start with a greeting.
    2.  Prioritize the most critical information first: high-priority cases and at-risk projects.
    3.  Then, summarize upcoming tasks and today's schedule.
    4.  If there's nothing pressing, provide a positive, encouraging message.
    5.  Keep it to a single, easy-to-read paragraph. Do not use markdown.
  `;
