...
const CaseTileSkeleton: React.FC = () => (
    <div className="bg-white/20 dark:bg-slate-900/40 backdrop-blur-xl p-5 rounded-lg border border-white/20 shadow-lg flex flex-col">
        ...
        <div className="mt-auto border-t border-slate-200 dark:border-slate-700 pt-3 space-y-2">
            ...
            <div className="flex justify-between pt-1"><Skeleton className="h-3 w-1/3" /><Skeleton className="h-3 w-1/4" /></div>
        </div>
    </div>
);
...
const CaseTile: React.FC<{ 
    ...
    return (
        <div className="bg-white/20 dark:bg-slate-900/40 backdrop-blur-xl p-5 rounded-lg border border-white/20 shadow-lg flex flex-col justify-between hover:border-indigo-400 dark:hover:border-indigo-600 transition-colors duration-200 group relative">
            ...
            <button onClick={onSelect} className="w-full h-full text-left flex flex-col">
                <div>
                    <div className="flex justify-between items-start mb-2">
...
const CaseCard: React.FC<{
    ...
    <div
        ...
        onClick={onSelect}
        className="bg-white/30 dark:bg-slate-800/50 p-3 rounded-md border border-white/20 dark:border-slate-700 cursor-grab active:cursor-grabbing hover:shadow-md hover:border-indigo-400 dark:hover:border-indigo-600"
    >
        ...
        <div className="text-shadow-strong">
                    ...
                    {view === 'tile' && (
                        <>
                            <select
                                ...
                                className="bg-white/50 dark:bg-black/30 border border-white/30 dark:border-white/20 rounded-md px-3 py-2 text-sm text-slate-700 dark:text-slate-200 focus:ring-indigo-500 focus:border-indigo-500"
                            >
                     ...
                     <div className="flex items-center gap-1 p-1 bg-black/10 rounded-lg border border-white/20 dark:bg-black/20 dark:border-white/10">
            ...
            {isLoading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {Array.from({ length: 8 }).map((_, i) => <CaseTileSkeleton key={i} />)}
                </div>
            ) : view === 'tile' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    {filteredCases.length === 0 && (
                        <div className="col-span-full text-center p-12 bg-white/30 dark:bg-black/20 backdrop-blur-xl rounded-lg border border-dashed border-white/20 dark:border-white/10 text-slate-500 dark:text-slate-400">
                ...
                <div className="flex gap-4 overflow-x-auto pb-4 -mx-6 sm:-mx-8 px-6 sm:px-8">
                    {Object.values(CaseStatus).map(status => (
                        <div 
                            key={status} 
                            className={`w-72 bg-slate-100/50 dark:bg-slate-900/40 rounded-lg p-3 flex-shrink-0 transition-colors ${draggedOverColumn === status ? 'bg-indigo-100 dark:bg-indigo-900/50' : ''}`}
