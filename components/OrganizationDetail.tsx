...
import { generateDonorInsights, DonorInsightsResult, generateFollowUpEmail } from '../src/services/geminiService';
...
interface EmailDraftModalProps {
    ...
    onContentChange: (newContent: { subject: string; body: string }) => void;
    onSend: () => void;
}

const EmailDraftModal: React.FC<EmailDraftModalProps> = ({ isOpen, onClose, isLoading, content, onContentChange, onSend }) => {
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Draft Email">
            {isLoading ? (
                <div className="flex flex-col items-center justify-center h-48">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-violet-500"></div>
                    <p className="mt-4 text-slate-500 dark:text-slate-400">Gemini is drafting your email...</p>
                </div>
            ) : (
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full p-2 bg-slate-100 border border-slate-300 rounded-md focus:ring-violet-500 focus:border-violet-500 dark:bg-slate-700/50 dark:border-slate-600"
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1 dark:text-slate-300">Body</label>
                        <textarea
                            value={content.body}
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        <button onClick={onClose} className="px-4 py-2 bg-white border border-slate-300 rounded-md text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200">Cancel</button>
                        <button onClick={onSend} className="px-4 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-md text-sm font-semibold hover:from-violet-700 hover:to-indigo-700">Send (Simulated)</button>
                    </div>
                </div>
            )}
        </Modal>
...
export const OrganizationDetail: React.FC<OrganizationDetailProps> = ({ client, projects, activities, cases, donations, documents, teamMembers, events, onBack, onSelectProject, onScheduleActivity, currentUserId }) => {
    ...
    const handleActionClick = async () => {
        ...
        const { actionType, actionTitle, text } = donorInsights.suggestion;
        ...
        } else if (actionType === 'DraftEmail') {
            setIsEmailModalOpen(true);
            setIsGeneratingEmail(true);
            setEmailContent({ subject: '', body: '' });
            
            const currentUser = teamMembers.find(tm => tm.id === currentUserId);
            // Use the full insight text for better context
            const fullInsightContext = `${donorInsights.insights} Actionable suggestion: ${text}`;

            try {
                const generatedEmail = await generateFollowUpEmail(client, fullInsightContext, currentUser?.name || 'Your Consultant');
    ...
    const handleSendEmail = () => {
        ...
        showToast(`Email to ${client.contactPerson} sent successfully! (Simulated)`, 'success');
