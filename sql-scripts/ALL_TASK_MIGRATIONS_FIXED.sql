-- ============================================
-- COMPLETE TASK MANAGEMENT MIGRATIONS (FIXED)
...
-- (Can be enhanced later with granular access control)
-- ============================================
...
-- SIMPLIFIED TASKS TABLE POLICIES (Permissive for all authenticated users)
...
-- TASK_SUBTASKS POLICIES (Simplified)

DROP POLICY IF EXISTS "Users can view subtasks of tasks they can view" ON task_subtasks;
DROP POLICY IF EXISTS "Users can add subtasks to tasks they can update" ON task_subtasks;
DROP POLICY IF EXISTS "Users can update subtasks of tasks they can update" ON task_subtasks;
DROP POLICY IF EXISTS "Users can delete subtasks of tasks they can update" ON task_subtasks;
...
CREATE POLICY "Authenticated users can add subtasks"
...
-- TASK_COMMENTS POLICIES (Simplified)

DROP POLICY IF EXISTS "Users can view comments on tasks they can view" ON task_comments;
DROP POLICY IF EXISTS "Users can add comments to tasks they can view" ON task_comments;
DROP POLICY IF EXISTS "Users can update their own comments" ON task_comments;
DROP POLICY IF EXISTS "Users can delete their own comments or admins can delete any" ON task_comments;
...
-- TASK_ATTACHMENTS POLICIES (Simplified)

DROP POLICY IF EXISTS "Users can view attachments on tasks they can view" ON task_attachments;
DROP POLICY IF EXISTS "Users can upload attachments to tasks they can update" ON task_attachments;
DROP POLICY IF EXISTS "Users can delete their own attachments or admins can delete any" ON task_attachments;
...
-- TASK_ACTIVITY POLICIES (Read-only for authenticated users)

DROP POLICY IF EXISTS "Users can view activity on tasks they can view" ON task_activity;
...
-- No INSERT/UPDATE/DELETE policies - activity log is managed by triggers only

-- TASK_DEPENDENCIES POLICIES (Simplified)

DROP POLICY IF EXISTS "Users can view dependencies between tasks they can view" ON task_dependencies;
DROP POLICY IF EXISTS "Users can create dependencies for tasks they can update" ON task_dependencies;
DROP POLICY IF EXISTS "Users can delete dependencies for tasks they can update" ON task_dependencies;
...
BEGIN
    ...
    RAISE NOTICE '  - Simplified RLS security policies (permissive for all authenticated users)';
    RAISE NOTICE '  - 1 helper view (tasks_with_metrics)';
    RAISE NOTICE '';
    RAISE NOTICE 'Next: Test the taskManagementService and connect TaskView.tsx';
    RAISE NOTICE '';
    RAISE NOTICE 'NOTE: RLS policies are currently permissive. You can enhance them later';
    RAISE NOTICE 'with granular role-based access control once you have a proper';
    RAISE NOTICE 'authentication and role management system in place.';
END $$;
